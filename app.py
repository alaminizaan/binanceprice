...
exchanges = {exchange_id: ccxt.__dict__[exchange_id]({
...
def home():
    try:
        # initialize empty list to store opportunities across all exchanges
        all_opportunities = []
        ...
        for exchange_id in ccxt.exchanges:
            try:
                ...
                # add opportunities for this exchange to the all_opportunities list
        ...
        # get current prices for all USDT markets across all exchanges
        tickers = {}
        for exchange_id in ccxt.exchanges:
            try:
                # create exchange object
                ...
                # get current prices for all USDT markets on this exchange
    ...
    tickers = {symbol: price for symbol, price in tickers.items() if symbol.split(':')[1] in [market['symbol'] for market in usdt_markets]}
...
def get_prices():
    try:
        ...
        # fetch the current prices for all markets
        prices = {}
